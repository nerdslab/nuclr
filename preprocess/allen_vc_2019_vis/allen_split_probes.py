...
root = Path("../data/processed/allen_vc_2019")
new_root = Path("../data/processed/allen_vc_2019_probes")
new_brainset_id = "allen_vc_2019_probes"
...
for i, probe in enumerate(probes):
    ...
    assert (new_data.units.unit_number == old_unit_idx).all()

    unit_idx_remap = np.zeros(len(data.units), dtype=int)
    unit_idx_remap[old_unit_idx] = np.arange(len(old_unit_idx))
    new_data.units.unit_number = unit_idx_remap[new_data.units.unit_number]
