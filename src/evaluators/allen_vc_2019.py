...
class AllenVC2019Evaluator:
    ...
    @staticmethod
    def prep_md_df():
        df = pd.read_csv("./neuron_metadata/allen_vc_2019.csv")
        unit_ids = [str(x) for x in df.unit_id]
        df["unit_id"] = unit_ids
        df["brain_region"] = [structure2region[x] for x in df.ecephys_structure_acronym]
    ...
    def __call__(
        ...
        # Sessionwise train brain region
        train_l1out_br_evaluator = LeaveOneSessionOutClassifier("train/sessCV_br", True)
        train_l1out_br_evaluator.launch(
            X=X_train_np[train_br_mask],
            y=train_br[train_br_mask],
            s=to_sw_cv(train_md.session_id[train_br_mask]),
        )
        self.futures.append(train_l1out_br_evaluator)
        ...
        # Sessionwise brain region
        val_l1out_br_evaluator = LeaveOneSessionOutClassifier("val/sessCV_br", True)
        val_l1out_br_evaluator.launch(
            X=X_val_np[val_br_mask],
            y=val_br[val_br_mask],
            s=to_sw_cv(val_md.session_id[val_br_mask]),
        )
        self.futures.append(val_l1out_br_evaluator)
